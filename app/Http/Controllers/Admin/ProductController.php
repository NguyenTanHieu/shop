...
class ProductController extends Controller
{
    ...
    public function create()
    {
        ...
        $tags = TagProduct::orderBy('name', 'ASC')->select('id', 'name')->get();
    ...
    {
        $cats = Category::orderBy('name', 'DESC')->select('id', 'name')->get();
        $tags = TagProduct::orderBy('name', 'DESC')->select('id', 'name')->get();
            ...
            if ($request->has('other_img')) {
                if ($product->images->count() > 0) {
                    foreach ($product->images as $img) {
                        $othe_imge = $img->image;
                        $other_path = public_path('uploads/product') . '/' . $othe_imge;
    ...
    public function searchproducts(Request $request)
    {
        try {
            ...
            $data = Product::where('name', 'like', $keyword . "%")->paginate(20);

            
            return response()->json([
                'status' => 200,
                'html' => view('admin.product.searchproduct', compact('data'))->render()
            ]);
